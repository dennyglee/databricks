# Databricks notebook source exported at Mon, 28 Mar 2016 16:03:46 UTC
# MAGIC %md # Site Analysis 
# MAGIC This notebook contains company website reporting and analysis.  This notebook does the following:
...
# MAGIC %sql select * from accessLogsPrime limit 10;
...
# MAGIC %sql select browserFamily, OSFamily, count(1) as events, count(distinct UserId) as users from accessLogsPrime where lcase(browserFamily) like '%mobile%' group by browserFamily, OSFamily
...
# MAGIC %sql select hour(datetime) as Hour, OSFamily, count(1) as events from accessLogsPrime where lcase(browserFamily) like '%mobile%' group by hour(datetime), OSFamily order by Hour 

# COMMAND ----------

# MAGIC %sql select OSFamily, count(distinct UserID) as Users, count(1) as Events from accessLogsPrime group by OSFamily order by Users desc limit 10;
...
# MAGIC %sql select responsecode, method, count(1) as sessions from accesslogsprime group by responsecode, method

# COMMAND ----------

# MAGIC %sql select parse_url(referrer, 'HOST') as host, count(1) as refers from accesslogsprime where parse_url(referrer, 'HOST') NOT IN ('[]', 'databricks.com')  group by parse_url(referrer, 'HOST') order by refers desc limit 10;