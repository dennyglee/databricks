// Databricks notebook source exported at Thu, 2 Jun 2016 05:04:50 UTC
...
// MAGIC * Timothy Danford [Presentations](http://www.slideshare.net/TimothyDanford)
...
import org.bdgenomics.formats.avro._   //this is not member of package org.bdgenomics. need ADAM cli
...
// MAGIC Note, the original source for the `6-sample.vcf` file is from http://med-at-scale.s3.amazonaws.com/samples/6-sample.vcf as part of Andy Petrella's Spark-Notebook project - [Local Analysis with Adam.snb](https://github.com/andypetrella/spark-notebook/blob/b7b25c1d14e840c2034f731c187cf43541d8fad2/notebooks/adam/Local%20Analysis%20with%20Adam.snb).   The original source of the `integrated_call_samples_v3.20130502.ALL.panel` is from the [1000 genomes project](http://www.1000genomes.org/) panel FTP site: [integrated_call_samples_v3.20130502.ALL.panel](ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/release/20130502/integrated_call_samples_v3.20130502.ALL.panel)
...
// MAGIC %md
// MAGIC Our VCF file contains the genotypes at locations which are variants, across many people (~8000? need to check). A quick view of the VCF file containing a subset of chromosome 6 data. This vcf file header contains 250 lines of metadata. The data we will need to analyze starts on line 251 of the file. 
...
// MAGIC %md  
// MAGIC After conversion to ADAM parquet files, our genotypes are now avro data models, and are stored as JSON. Here is what one record looks like for a variant which was found to have alleles G and C in all of our data, but the person HG00110 has genotype G/G (Ref,Ref):
...
// MAGIC %md
// MAGIC Our sample panel file was obtained from:
// MAGIC <pre>ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/release/20130502/integrated_call_samples_v3.20130502.ALL.panel</pre>
// MAGIC 
// MAGIC It contains columns for person ID (sample), population code (pop), super population code (super_pop), and gender.  This will be our lookup panel for each person's population membership for our supervised learning model.
// MAGIC 
// MAGIC Note, the population code definitions can be found at http://www.1000genomes.org/cell-lines-and-dna-coriell.
// MAGIC 
...
// MAGIC %md 
// MAGIC If a variant is missing data, the number of genotypes per variant will be lower than the number of people. If a variant is triallelic, the number will be higher. 
