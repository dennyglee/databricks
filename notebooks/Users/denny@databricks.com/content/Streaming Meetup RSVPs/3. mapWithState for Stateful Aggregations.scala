// Databricks notebook source exported at Mon, 28 Mar 2016 15:43:02 UTC
// MAGIC %md ## mapWithState for Stateful Aggregations
...
val initialRDD = sc.parallelize(List(("CA", 0L), ("WA", 0L)))
...
// MAGIC %md ### Extract groupState Function
// MAGIC Extract groupState from the Meetup JSON using json4s (Jackson)
...
def parseRsvpGroupState(rsvpJson: String):String = {
  Try({
    val json = parse(rsvpJson).camelizeKeys
    val groupCountry = compact(render(json \\ "group" \\ "groupCountry")).replace("\"", "")
    val groupState = compact(render(json \\ "group" \\ "groupState")).replace("\"", "")
          
    if (groupCountry == "us") groupState
    else "{}"
  }).toString
}
...
def creatingFunc(): StreamingContext = {
  ...
  val meetupStream = stream.map(line => parseSuccess(parseRsvpGroupState(line))).map(groupState => (groupState, 1))
  ...
  stateSnapshotStream.foreachRDD { rdd =>
    rdd.toDF("group_state", "count").registerTempTable("batch_meetup_stream")
  }
  
  ssc.remember(Minutes(1))  // To make sure data is not deleted by the time we query it interactively
  
  ssc.checkpoint("dbfs:/streaming/trackstate/119")
...
// MAGIC %sql select group_state as `State Code`, count as `RSVPs` from batch_meetup_stream where group_state <> '{}' order by count desc
