...
# MAGIC %scala
# MAGIC val parquetPath = "/tmp/sais_eu_19_demo/loans/"
# MAGIC 
# MAGIC // Configurations necessary for running of Databricks Community Edition
# MAGIC spark.sql("set spark.sql.shuffle.partitions = 1")
# MAGIC spark.sql("set spark.databricks.delta.snapshotPartitions = 1")
# MAGIC 
# MAGIC // Create a view on the table called loans_parquet
# MAGIC spark.read.format("parquet").load(parquetPath).createOrReplaceTempView("loans_parquet")
# MAGIC println("Defined view 'loans_parquet'")
...
# MAGIC 
# MAGIC parquet_path = "/tmp/sais_eu_19_demo/loans/"
# MAGIC 
# MAGIC # Create a view on the table called loans_parquet
# MAGIC spark.read.format("parquet").load(parquet_path).createOrReplaceTempView("loans_parquet")
# MAGIC print("Defined view 'loans_parquet'")
...
# MAGIC 
# MAGIC 
# MAGIC val random = new Random()
# MAGIC 
# MAGIC def randomCheckpointDir: String = 
# MAGIC   s"/tmp/sais_eu_19_demo/chkpt/${random.nextInt(10000)}"
# MAGIC 
...
# MAGIC     .withColumn("loan_id", col("value") + 10000)
# MAGIC     .withColumn("funded_amnt", (rand() * 5000 + 5000).cast("integer"))
# MAGIC     .withColumn("paid_amnt", col("funded_amnt") - (rand() * 2000))
# MAGIC     .withColumn("addr_state", randomState())
# MAGIC 
# MAGIC   val query = streamData.writeStream
# MAGIC     .format(tableFormat)
# MAGIC     .option("checkpointLocation", randomCheckpointDir)
# MAGIC     .trigger(Trigger.ProcessingTime("10 seconds"))
# MAGIC     .start(tablePath)
# MAGIC 
# MAGIC   query
# MAGIC }
# MAGIC 
...
# MAGIC   println("Stopping all streams")
# MAGIC   spark.streams.active.foreach(_.stop())
# MAGIC   println("Stopped all streams")
# MAGIC   println("Deleting checkpoints")  
# MAGIC   dbutils.fs.rm("/sais_eu_19_demo/chkpt/", true)
# MAGIC   println("Deleted checkpoints")
# MAGIC }
...
# MAGIC   return "/tmp/sais_eu_19_demo/chkpt/%s" % str(random.randint(0, 10000))
# MAGIC 
# MAGIC # User-defined function to generate random state
# MAGIC 
# MAGIC states = ["CA", "TX", "NY", "WA"]
# MAGIC 
...
# MAGIC     .withColumn("loan_id", 10000 + col("value")) 
# MAGIC     .withColumn("funded_amnt", (rand() * 5000 + 5000).cast("integer")) 
# MAGIC     .withColumn("paid_amnt", col("funded_amnt") - (rand() * 2000)) 
...
# MAGIC %scala
# MAGIC val streamQuery = generateAndAppendDataStream(
# MAGIC     tableFormat = "parquet", 
# MAGIC     tablePath = parquetPath)
...
# MAGIC val deltaPath = "/tmp/sais_eu_19_demo/loans_delta"
# MAGIC 
# MAGIC // Configurations necessary for running of Databricks Community Edition
# MAGIC spark.sql("set spark.sql.shuffle.partitions = 1")
# MAGIC spark.sql("set spark.databricks.delta.snapshotPartitions = 1")
...
# MAGIC println("Deleting directory " + deltaPath)
# MAGIC dbutils.fs.rm(deltaPath, recurse = true)
# MAGIC 
# MAGIC // Create the Delta table with the same loans data
# MAGIC spark.read.format("parquet").load(parquetPath)
# MAGIC   .write.format("delta").save(deltaPath)
# MAGIC println("Created a Delta table at " + deltaPath)
# MAGIC 
# MAGIC spark.read.format("delta").load(deltaPath).createOrReplaceTempView("loans_delta")
# MAGIC println("Defined view 'loans_delta'")
...
# MAGIC 
# MAGIC # Configure Delta Lake Silver Path
# MAGIC delta_path = "/tmp/sais_eu_19_demo/loans_delta"
# MAGIC 
# MAGIC # Configurations necessary for running of Databricks Community Edition
# MAGIC spark.sql("set spark.sql.shuffle.partitions = 1")
# MAGIC spark.sql("set spark.databricks.delta.snapshotPartitions = 1")
# MAGIC 
# MAGIC # Remove folder if it exists
# MAGIC print("Deleting directory " + delta_path)
# MAGIC dbutils.fs.rm(delta_path, recurse=True)
# MAGIC 
# MAGIC # Create the Delta table with the same loans data
# MAGIC spark.read.format("parquet").load(parquet_path) \
# MAGIC   .write.format("delta").save(delta_path)
# MAGIC print("Created a Delta table at " + delta_path)
# MAGIC 
# MAGIC spark.read.format("delta").load(delta_path).createOrReplaceTempView("loans_delta")
# MAGIC print("Defined view 'loans_delta'")
...
# MAGIC %scala
# MAGIC val streamQuery2 = generateAndAppendDataStream(tableFormat = "delta", tablePath = deltaPath) 

# COMMAND ----------

# MAGIC %python
# MAGIC stream_query_2 = generate_and_append_data_stream(table_format = "delta", table_path = delta_path)
...
# MAGIC %scala
# MAGIC // Generate a stream of randomly generated load data and append to the parquet table
# MAGIC def generateAndAppendDataStreamFixed(tableFormat: String, tablePath: String): StreamingQuery = {
# MAGIC   
# MAGIC   val streamData = spark.readStream.format("rate").option("rowsPerSecond", 5).load()
# MAGIC     .withColumn("loan_id", col("value") + 10000)
# MAGIC     .withColumn("funded_amnt", (rand() * 5000 + 5000).cast("integer"))
# MAGIC     .withColumn("paid_amnt", col("funded_amnt") - (rand() * 2000))
# MAGIC     .withColumn("addr_state", randomState())
# MAGIC     .select("loan_id", "funded_amnt", "paid_amnt", "addr_state")   // *********** FIXED THE SCHEMA OF THE GENERATED DATA *************
# MAGIC 
# MAGIC   val query = streamData.writeStream
# MAGIC     .format(tableFormat)
# MAGIC     .option("checkpointLocation", randomCheckpointDir)
...
# MAGIC 
# MAGIC # Generate a stream of randomly generated load data and append to the parquet table
# MAGIC def generate_and_append_data_stream_fixed(table_format, table_path):
# MAGIC     
# MAGIC   stream_data = (spark.readStream.format("rate").option("rowsPerSecond", 50).load() 
# MAGIC     .withColumn("loan_id", 10000 + col("value")) 
# MAGIC     .withColumn("funded_amnt", (rand() * 5000 + 5000).cast("integer")) 
# MAGIC     .withColumn("paid_amnt", col("funded_amnt") - (rand() * 2000)) 
# MAGIC     .withColumn("addr_state", random_state()) 
...
# MAGIC %scala
# MAGIC val streamQuery2 = generateAndAppendDataStreamFixed(tableFormat = "delta", tablePath = deltaPath)

# COMMAND ----------

# MAGIC %python
# MAGIC stream_query_2 = generate_and_append_data_stream_fixed(table_format = "delta", table_path = delta_path)
...
# MAGIC %scala
# MAGIC val streamQuery3 = generateAndAppendDataStreamFixed(tableFormat = "delta", tablePath = deltaPath)

# COMMAND ----------

# MAGIC %python
# MAGIC stream_query_3 = generate_and_append_data_stream_fixed(table_format = "delta", table_path = delta_path)
...
# MAGIC   (1111111L, 1000, 1000.0, "TX", false), 
# MAGIC   (2222222L, 2000, 0.0, "CA", true))
# MAGIC   .toDF("loan_id", "funded_amnt", "paid_amnt", "addr_state", "closed")
# MAGIC   
# MAGIC loanUpdates.write.format("delta")
# MAGIC   .mode("append")
# MAGIC   .option("mergeSchema", "true")
# MAGIC   .save(deltaPath)

# COMMAND ----------

# MAGIC %python
# MAGIC cols = ['loan_id', 'funded_amnt', 'paid_amnt', 'addr_state', 'closed']
# MAGIC 
# MAGIC items = [
# MAGIC   (1111111, 1000, 1000.0, 'TX', True), 
# MAGIC   (2222222, 2000, 0.0, 'CA', False)
# MAGIC ]
# MAGIC 
# MAGIC loan_updates = spark.createDataFrame(items, cols) \
# MAGIC   .withColumn("funded_amnt", col("funded_amnt").cast("int"))
# MAGIC   
# MAGIC loan_updates.write.format("delta") \
# MAGIC   .mode("append") \
# MAGIC   .option("mergeSchema", "true") \
...
# MAGIC %md ## ![Delta Lake Logo Tiny](https://pages.databricks.com/rs/094-YMS-629/images/delta-lake-tiny-logo.png) Update a Delta Lake table
...
# MAGIC   col("addr_state") === "OR",
# MAGIC   Map("addr_state" -> lit("WA")))
# MAGIC ;
...
# MAGIC deltaTable.update("addr_state = 'OR'",  {"addr_state":"'WA'"})
...
# MAGIC %md ## ![Delta Lake Logo Tiny](https://pages.databricks.com/rs/094-YMS-629/images/delta-lake-tiny-logo.png) Delete from Delta Lake table
# MAGIC 
...
# MAGIC %md Let's see how many files exist?
